...
print("\n\n#####################################################################################################")
print("\n\nData processing script for MRtrix3 - Coded by Diego ALVES RODRIGUES DE SOUZA - Modified by Arno ROSSEZ")
print("\n\n#####################################################################################################")
...
#1-CHOOSING HOW TO PROCEED (FILE INPUT OR MANUAL INPUT)---------------------------------------------------------------
...
if (inputByFile):
    ...
    print(important)
    important=[w.replace('QuantityOfSamples: ', '').replace('VoxelDimensions(mm): ', '').replace('NumberOfDirections: ', '').replace('NumberOfSeedsperVoxel: ', '').replace('FractionalAnisotropyCutoff: ', '').replace('StepSize(mm): ', '').replace('Curvature: ', '').replace('PerformN4BiasonDWIVolumes: ','').replace('ViewPreprocessingResultsduringRun: ','').replace('SphericalDeconvolutionResponseAlgorithm: ','').replace('TractographyAlgorithm: ','').replace('ROIsToBeAnalysed: ','').replace('PerformROIFiltering: ','').replace('PerformROIStatistics: ','').replace('PerformWholeBrainStatistics: ','').replace('PerformSIFTOnly: ','').replace('SIFTmode: ','').replace('SIFT1TerminationOptions: ','').replace('SIFT1TerminationValues: ','').replace('WorkPath: ','').replace('ProvideListOfImages: ','').replace('ProvideExternalMask: ','').replace('SameMaskforAllImages: ','').replace('SameBvecsBvalsforAllImages: ','').replace('SubFolderComment: ','').replace('NumberOfThreads: ','').replace('\n', '') for w in important]
    important=[w.replace('QuantityOfSamples:', '').replace('VoxelDimensions(mm):', '').replace('NumberOfDirections:', '').replace('NumberOfSeedsperVoxel:', '').replace('FractionalAnisotropyCutoff:', '').replace('StepSize(mm):', '').replace('Curvature:', '').replace('PerformN4BiasonDWIVolumes:','').replace('ViewPreprocessingResultsduringRun:','').replace('SphericalDeconvolutionResponseAlgorithm:','').replace('TractographyAlgorithm:','').replace('ROIsToBeAnalysed:','').replace('PerformROIFiltering:','').replace('PerformROIStatistics:','').replace('PerformWholeBrainStatistics:','').replace('PerformSIFTOnly:','').replace('SIFTmode:','').replace('SIFT1TerminationOptions:','').replace('SIFT1TerminationValues:','').replace('WorkPath:','').replace('ProvideListOfImages:','').replace('ProvideExternalMask:','').replace('SameMaskforAllImages:','').replace('SameBvecsBvalsforAllImages:','').replace('SubFolderComment:','').replace('NumberOfThreads:','').replace('\n', '') for w in important]
    ...
    provideListOfImages = str2bool(important[20])
    externalMask = str2bool(important[21])
    sameExternalMask = str2bool(important[22])
    sameBvecsBvals = str2bool(important[23])
    folderComment = important[24]

    nThreads = int(important[25])
...
#Terminating code if no input parameter file is given...
else:
    print("Input by file not chosen, code will thus be terminated")
    message = messagebox.showinfo("TERMINATION OF SCRIPT","CODE WILL BE TERMINATED AS THERE IS NO INPUT PARAMETERS TO WORK WITH...")
    exit()

#1-END-----------------------------------------------------------------------------------------------------------------------------

#2-SELECTING IMAGES, BVECS-BVALS & MASKS-------------------------------------------------------------------------------------------
if ((not(performROIFiltering)) and (not(performSIFTOnly)) and not(performROIStatistics) and not(performWholeBrainStatistics)):
    ...
    while (quantityFlag > 0):
        if not (provideListOfImages):
            instruction = ("Choose your .nii image file of the sample " + str(quantityofSamples-quantityFlag+1))
            niiFilePath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("NIfTI Image files", ".nii")])

            if not (sameBvecsBvals):
                instruction = ("Choose your .txt file containing Bvecs&Bvals for the sample " + str(quantityofSamples-quantityFlag+1))
                bvecBvalFilePath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("BvecsBvals Text files", ".txt")])

            if (sameBvecsBvals and (quantityFlag==quantityofSamples)):
                instruction = "Choose your single .txt file containing Bvecs&Bvals for all your samples..."
                bvecBvalFilePath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("BvecsBvals Text files", ".txt")])
            ...
            if((externalMask) and (not(sameExternalMask))):
                instruction = ("Choose your .nii mask file for the sample " + str(quantityofSamples - quantityFlag+1))
                externalMaskPath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("NIfTI Mask files", ".nii")])
                externalMaskPathVector.append(externalMaskPath)                

            if((externalMask) and (sameExternalMask) and (quantityFlag==quantityofSamples)):
                instruction = "Choose your single .nii mask file for all your samples..."
                externalMaskPath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("NIfTI Mask files", ".nii")])
                externalMaskPathVector.append(externalMaskPath)
            ...
            niiFilePathVector.append(niiFilePath)
            bvecBvalFilePathVector.append(bvecBvalFilePath)


        else:
            if (quantityFlag == quantityofSamples):
                instruction = ("Choose a .txt file containing the paths to all " + str(quantityofSamples) + " images to be processed...(inputImage.txt)")
                allImagesTxtDocPath = filedialog.askopenfilename(initialdir = workPath, title=instruction, filetypes=[("Input Text files", ".txt")])

                with open(allImagesTxtDocPath, 'r', encoding='utf-8', errors='replace') as file:
                    file = file.readlines()
                niiFilePathVectorTemp = file
    ...
    if (provideListOfImages):
        niiFilePathVector = [x[:-1] for x in niiFilePathVectorTemp[0:quantityofSamples]]
        print("\nNii filepath temp vector:")
        print(len(niiFilePathVectorTemp))
        print(niiFilePathVectorTemp)
...
#3-SELECTING OUTPUT FOLDER & COPYING IN USED PARAMETERS-----------------------------------------------------------------------------------

    instruction = "Choose an output folder for the tractograpy..."
    outputPath = filedialog.askdirectory(initialdir = workPath, title=instruction)
...
if (performSIFTOnly and not(SIFTAborted)):
    ...
    print("\n#######")
    message = messagebox.showinfo("Asking for TractographyResults folder", "Choose the output folder already containing the data of all " + str(quantityofSamples) + " subjects to be re-processed...")
...
ROIasking = messagebox.askyesno("Proceeding with ROI Analysis", "Do you want to do ROI Analysis to the current tractography ? (Provided there are ROIs available)")
...
if not(ROIasking):
     print("\n\n####################################################################################################")
     print("\n user chose : Skipping ROI Filtering step")
     print("\n######################################################################################################")
     
elif (ROIasking and performROIFiltering) or (ROIasking and performROIStatistics):
    print("\n\n####################################################################################################")
    ...
    message = messagebox.showinfo("Choice of ready output folder", "Choose the output folder already containing the data of all " + str(quantityofSamples) + " subjects to be processed...")
    newOutputPath = filedialog.askdirectory(initialdir=workPath)         
